	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(page.equals("sign-up-form")) {
			...
			String password_2 = request.getParameter("password_2");
			 ...
			 if(password_1.length() < 1 || password_2.length() < 1 )retVal.append("Empty fields <br>");
				...
				if (password_1 != null && password_2 != null) {
		            ...
		            if (password_1.equals(password_2)) {
		                ...
		                System.out.println(password_1 + " = " + password_2);

		                password_1 = password_2;
		                boolean hasUppercase = !password_1.equals(password_1.toLowerCase());
		                boolean hasLowercase = !password_1.equals(password_1.toUpperCase());
		                boolean hasNumber = password_1.matches(".*\\d.*");
		                boolean noSpecialChar = password_1.matches("[a-zA-Z0-9 ]*");

		                if (password_1.length() < 11) {
		                	 System.out.println(password_1 + " is length < 11");
		                    retVal.append("Password is too short. Needs to have 11 characters <br>");
		                }

		                if (!hasUppercase) {
		                	 System.out.println(password_1 + " <-- needs uppercase");
		                    retVal.append("Password needs an upper case <br>");
		                }

		                if (!hasLowercase) {
		                	 System.out.println(password_1 + " <-- needs lowercase");
		                    retVal.append("Password needs a lowercase <br>");
		                }

		                if (!hasNumber) {
		                	 System.out.println(password_1 + "<-- needs a number");
		                    retVal.append("Password needs a number <br>");
		                }

		                if(noSpecialChar){
		                	 System.out.println(password_1 + "<-- needs a specail character");
